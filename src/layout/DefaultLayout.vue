...
<template>
    <div class="flex w-full h-full">
        ...
        <div ref="main" @scroll="handleScroll" class="bg-[#f5f7fb] transition-all duration-500 ease-in-out sm:px-4 pb-4 overflow-x-hidden max-h-screen" :class="smallBar ? 'px-1 w-full' : 'w-0 sm:w-full'">
            <div class="flex w-full items-center justify-between py-4 sticky top-0 z-10 transition-all duration-500 ease-in-out" :class="scrollPosition > 0 ? 'bg-[#52a1f5] text-white px-4 shadow-lg rounded-b-2xl' : 'bg-[#f5f7fb] text-[#52a1f5]'">
                <p class="text-xl font-bold ml-1">MTHP Dashboard</p>
